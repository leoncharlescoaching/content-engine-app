  ...
  const run = async () => {
    ...
    try {
      ...
      if (tool.id === "hook") {
        ...
        result = await callClaude(PROMPTS.ctaBuilder, input || "Fat loss coaching for busy dads");
      } else {
        const userMsg = `Intent: ${intent}\n${hook ? `Hook: ${hook}\n` : ""}Content: ${input || "Generate a strong example."}`;
        result = await callClaude(PROMPTS[tool.promptKey], userMsg);
      }
        ...
        <SoftPanel>
                ...
                >
                  <div>
                    ...
                    <div
                      style={{
                        fontSize: 11.5,
                        color: active ? "#4b4b52" : MUTED,
                      }}
                    >
                      {t.desc}
                    </div>
                  </div>
                  {active ? <span style={{ color: RED, fontWeight: 900 }}>✓</span> : null}
                </button>
              );
            })}
          </div>
              ...
              {tool.id === "caption" ? (
                <>
                  ...
                  <TextInput
                    value={hook}
                    onChange={setHook}
                    placeholder="Paste a hook or leave blank"
                    rows={2}
                  />
                </>
              ) : null}
          ...
          {output && !loading ? (
            <div style={{ animation: "fadeUp 0.2s ease" }}>
              <div style={{ height: 1, background: BORDER, margin: "18px 0" }} />
              <Card title="Hook" content={output.hook} accent />
              <Card title="Body" content={output.body} />
              <Card title="CTA" content={output.cta} />
              <Card title="Comment CTAs" content={output.comment} />
              <Card title="DM CTAs" content={output.dm} />
              <Card title="Engagement CTAs" content={output.engagement} />
              <Card title="Refined Hooks" content={output.refined} />
              <Card title="Strongest" content={output.strongest} accent />
              <Card title="Why Strongest" content={output.whyStrongest} />
              <Card title="Rewritten" content={output.rewritten} accent />
              <Card title="What Changed" content={output.whatChanged} />
              <Card title="Opening Hook" content={output.openingHook} accent />
...
function YouTubeExpansionScreen({ onBack }) {
  ...
  function YTBlock({ title, content, accent = false }) {
    if (!content || (Array.isArray(content) && content.length === 0)) return null;
    return <Card title={title} content={content} accent={accent} mono={false} />;
  }
  ...
  return (
    <AppShell>
      ...
      <div style={{ padding: "0 16px 24px" }}>
        {!output ? (
          <SoftPanel>
            <p style={{ margin: "0 0 14px", color: MUTED, fontSize: 13.5, lineHeight: 1.7 }}>
              Drop a topic or idea. Get titles, structure, clips, hooks, captions, series and scores.
            </p>

            <TextInput
              value={input}
              onChange={setInput}
              placeholder="e.g. why dads lose muscle faster after 35"
              rows={5}
            />
        ...
        {loading && !output ? <Spinner msg={msg} /> : null}

        {output && !loading ? (
          <>
            <ScoreBar score={output.finalScores?.viralScore} label="Viral Score" />
            <ScoreBar score={output.finalScores?.conversionScore} label="Conversion Score" />

            <YTBlock title="Angle" content={output.coreIdea?.angle} />
            <YTBlock title="Who It's For" content={output.coreIdea?.whoItsFor} />
            <YTBlock title="Main Message" content={output.coreIdea?.mainMessage} />
            <YTBlock title="Best Content Type" content={output.coreIdea?.bestContentType} accent />
            <YTBlock title="YouTube Titles" content={output.youtubeTitles} />
            <YTBlock title="YouTube Description" content={output.youtubeDescription} />
            <YTBlock title="Video Hook" content={output.videoStructure?.hook} />
            <YTBlock title="Intro" content={output.videoStructure?.intro} />
            <YTBlock title="Key Points" content={output.videoStructure?.keyPoints} />
            <YTBlock title="Real-Life Examples" content={output.videoStructure?.realLifeExamples} />
            <YTBlock title="Closing Message" content={output.videoStructure?.closingMessage} />
            <YTBlock title="Short Form Clips" content={output.shortFormClips?.map((c) => `${c.hook} — ${c.idea}`)} />
            <YTBlock title="Opening Line" content={output.firstThreeSeconds?.openingLine} accent />
            <YTBlock title="Visual Idea" content={output.firstThreeSeconds?.visualIdea} />
            <YTBlock title="On Screen Text" content={output.firstThreeSeconds?.onScreenText} />
            <YTBlock title="Hooks" content={output.hookSystem?.hooks} />
            <YTBlock title="Best Hook" content={output.hookSystem?.bestHook} accent />
            <YTBlock title="Weakest Hook Improved" content={output.hookSystem?.weakestHookImproved} />
            <YTBlock title="Thumbnail Options" content={output.thumbnailOptions} />
            <YTBlock title="Comment Bait" content={output.commentBait} />
            <YTBlock title="Content Series" content={output.contentSeries?.map((x) => `${x.hook} — ${x.angle} — ${x.idea}`)} />
            <YTBlock title="Instagram Caption Hook" content={output.instagramCaption?.hook} />
            <YTBlock title="Instagram Caption Body" content={output.instagramCaption?.body} />
            <YTBlock title="Instagram CTA" content={output.instagramCaption?.cta} />
            <YTBlock title="TikTok Caption" content={output.tiktokCaption} />
            <YTBlock title="What Is Weak" content={output.contentImprovement?.whatIsWeak} />
            <YTBlock title="What To Remove" content={output.contentImprovement?.whatToRemove} />
            <YTBlock title="Stronger Version" content={output.contentImprovement?.strongerVersion} accent />
        ...
        {saved.length === 0 ? (
          ...
          saved.map((item, i) => (
            ...
            >
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  <span
                    style={{
                      ...
                      letterSpacing: "0.07em",
                      textTransform: "uppercase",
                      color: item.lane === "coaching" ? RED : TEXT,
                      background: item.lane === "coaching" ? SOFT_RED : SURFACE_2,
                      padding: "4px 8px",
                      borderRadius: 999,
                    }}
                  >
              ...
              <p style={{ margin: "0 0 5px", color: TEXT, fontSize: 14, fontWeight: 800, lineHeight: 1.45 }}>
              ...
              <p style={{ margin: "8px 0 0", color: MUTED_2, fontSize: 11.5 }}>{item.date}</p>
            </div>
          ))
        )}
      </div>
...
export default function App() {
  ...
  const handleOutput = (result, laneValue, formatValue, showLast = false) => {
    if (showLast) {
      setScreen("output");
      return;
    }
    ...
    setSaved((prev) => [
      {
        lane: outputLane,
        format: outputFormat,
        topic: item.topic || item.title || item.factOrAngle || item.fact || "",
  ...
  return (
    <>
      ...
      {screen === "copy" ? <CopySystemScreen onBack={() => setScreen("home")} /> : null}
      {screen === "youtube" ? <YouTubeExpansionScreen onBack={() => setScreen("home")} /> : null}
